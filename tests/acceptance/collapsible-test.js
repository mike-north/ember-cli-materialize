...
test('Collapsible basic tests', function(assert) {
  ...
  andThen(function() {
    ...
    assert.equal(
      find('.accordion-example ul > li>.active').length,
      0,
      'Accordion should not have an active collapsible'
    );
    assert.equal(
      find('.expandable-example ul > li>.active').length,
      0,
      'Expandable should not have an active collapsible'
    );
    assert.equal(
      find('.preselected-example ul > li>.active').length,
      1,
